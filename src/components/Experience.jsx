import React from "react";
import { EXPERIENCES } from "../constants";
import { motion } from "framer-motion";

function Experience() {
  return (
    <div className="border-b border-neutral-700 pb-4">
      <h1 className="my-20 text-center text-5xl ">
        Work <span className="text-neutral-500">Experience</span>
      </h1>
      <div>
        {EXPERIENCES.map((experience, index) => (
          <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
            <motion.div
              whileInView={{ opacity: 1, x: 0 }}
              initial={{ opacity: 0, x: -100 }}
              transition={{ duration: 1 }}
              className="w-full lg:w-1/4"
            >
              <p className="mb-2 text-lg bg-gradient-to-r from-pink-300 via-slate-500 to-purple-500 bg-clip-text  tracking-tight text-transparent">
                {experience.year}
              </p>
            </motion.div>

            <div className="w-full max-w-xl lg:w-3/4">
              <h6 className="mb-2 font-semibold text-lg">
                {experience.role} - <span>{experience.company}</span>
              </h6>

              <p className="mb-4 text-neutral-400">{experience.description}</p>

              <div className="flex-col whitespace-nowrap">
                {experience?.technologies?.map((technology, index) => (
                  <span
                    key={index}
                    className="mr-2 mt-4 rounded bg-neutral-900 px-2 py-1 text-sm font-medium text-purple-500"
                  >
                    {technology}
                  </span>
                ))}

                {experience?.technologies2?.map((technology2, index) => (
                  <span
                    key={index}
                    className="mr-2 mt-6 rounded bg-neutral-900 px-2 py-1 text-sm font-medium text-purple-500"
                  >
                    {technology2}
                  </span>
                ))}
              </div>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}

export default Experience;
